...
while(num>0){name=chars[num%chars.length]+name;num=Math.floor(num/chars.length);}
return '_'+name;
}
...
// ==========================================
// XOR & CONSTANT TABLE SYSTEM
// ==========================================
xorEncrypt(str){
const result=[];
for(let i=0;i<str.length;i++){result.push(str.charCodeAt(i)^this.xorKey);}
return result;
}
...
// FIX: Added spaces around keywords to prevent minification errors
...
if(this.constantMap.has(str)){return this.constantMap.get(str);}
...
if((c==='"'||c==="'")&&prev!=='\\'){
if(!inStr){inStr=true;q=c;}else if(c===q){inStr=false;}
}
if(!inStr&&c==='-'&&line[i+1]==='-'){count++;return line.slice(0,i).trimEnd();}
}
return line;
}).join('\n');
...
const vars=[];
let m;
// Max Security now uses Short Vars (Luraph style)
const useShort=this.preset==='maxSecurity'; 
...
result=result.replace(re,(match,offset)=>{return this.isInString(result,offset)?match:v.new;});
}
...
const q=code[i];let content='';i++;
while(i<code.length){
if(code[i]==='\\'&&i+1<code.length){content+=code[i]+code[i+1];i+=2;}
else if(code[i]===q){break;}
else{content+=code[i];i++;}
}
i++;
...
if(isSafe){result+=this.encodeString(content);encoded++;}
else{result+=q+content+q;}
...
while(i<code.length){
if((code[i]==='"'||code[i]==="'")&&(i===0||code[i-1]!=='\\')){
const q=code[i];const startPos=i;let content='';i++;
while(i<code.length){
if(code[i]==='\\'&&i+1<code.length){content+=code[i]+code[i+1];i+=2;}
else if(code[i]===q){break;}
else{content+=code[i];i++;}
...
if(isSafe){strings.push({start:startPos,end:i,content:content,quote:q});}
}else{i++;}
}
return strings;
}
...
strings.forEach(s=>{this.addToConstantTable(s.content);});
...
if(prev==='.'||next==='.'||prev==='_'||prev==='x'||prev==='X'){return match;}
...
for(let i=0;i<this.rand(2,3);i++){newLines.push(deadPatterns[this.rand(0,deadPatterns.length-1)]());injected++;}
for(let i=0;i<lines.length;i++){
newLines.push(lines[i]);
if(i>0&&i<lines.length-1&&this.rand(1,100)<=8){
newLines.push(deadPatterns[this.rand(0,deadPatterns.length-1)]());injected++;
}
}
...
// ==========================================
// FIXED MINIFIER (SAFE SPACE)
// ==========================================
minifySingleLine(code){
let result=code;
// 1. Replace newlines with spaces (safe)
result=result.replace(/[\r\n]+/g,' ');
// 2. Reduce multiple spaces to one
result=result.replace(/\s+/g,' ');
// 3. Remove spaces around safe operators only (FIXED LIST)
// Removed '#' from list to prevent merging with keywords
result=result.replace(/\s*([=+\-*/%^,{}()\[\];])\s*/g,'$1');
// 4. Manual check: ensure space after keywords if next char is alphanumeric
const keywords=['and','or','not','local','function','if','then','else','elseif','end','for','while','do','repeat','until','return','in','true','false','nil','break'];
keywords.forEach(kw=>{
// Regex: Keyword followed by Space + WordChar -> Keyword + Space + WordChar
// This ensures 'do _R' stays 'do _R', not 'do_R'
const re=new RegExp('\\b'+kw+'\\s+([a-zA-Z0-9_])','g');
...
// MAX SECURITY NOW HAS TIER 1 FEATURES
...
code=this.cleanCode(code); // Clean newlines before minify
// Insert Decryptor & Table
const decryptor=this.generateDecryptor();
const constTable=this.generateConstantTable();
if(this.constantTable.length>0){
// Add semicolons to separate blocks safely
...
const app=express();
app.get('/',(req,res)=>{
res.send('<!DOCTYPE html><html><head><title>LuaGuard v5.4</title><style>body{font-family:Arial;background:#0d1117;color:#c9d1d9;text-align:center;padding:50px}h1{color:#58a6ff}.ok{color:#3fb950;font-size:1.3em}.box{background:#161b22;padding:25px;border-radius:12px;max-width:500px;margin:25px auto;border:1px solid #30363d}ul{text-align:left;padding-left:20px}li{margin:8px 0}</style></head><body><h1>LuaGuard v5.4</h1><p class="ok">● Online</p><div class="box"><p><b>Max Security Now Includes:</b></p><ul><li>✅ XOR String Encryption</li><li>✅ Constant Table Storage</li><li>✅ Safe Minification (1-Line)</li><li>✅ Number Obfuscation</li><li>✅ Dead Code Injection</li></ul></div><p style="color:#8b949e">Delta Compatible</p></body></html>');
});
...
const commands=[
new SlashCommandBuilder().setName('obfuscate').setDescription('Obfuscate Lua script')
...
if(CLIENT_ID){
try{await rest.put(Routes.applicationCommands(CLIENT_ID),{body:commands});console.log('[Bot] Commands OK');}
...
if(interaction.commandName==='ping'){return interaction.reply('🏓 '+(Date.now()-interaction.createdTimestamp)+'ms');}

if(interaction.commandName==='help'){
const embed=new EmbedBuilder().setColor(0x58a6ff).setTitle('🛡️ LuaGuard v5.4')
.addFields(
{name:'⚡ Performance',value:'```Clean + Comments```',inline:true},
{name:'⚖️ Balanced',value:'```+ Vars + Strings```',inline:true},
{name:'🔒 Max Security',value:'```+ XOR + ConstTable + Minify```',inline:true},
{name:'🔥 Update v5.4',value:'Max Security now uses Luraph-style protection (Encryption + Compression)',inline:false}
)
...
if(!['.lua','.luau','.txt'].some(e=>file.name.toLowerCase().endsWith(e))) return interaction.reply({content:'❌ Invalid file',ephemeral:true});
...
.setColor(preset==='maxSecurity'?0xf85149:0x58a6ff)
.setTitle('🛡️ Obfuscation Complete')
.addFields(
